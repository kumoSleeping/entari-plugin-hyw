...
def compress_image_b64(b64_data: str, quality: int = 90, max_width: int = 2048) -> str:
    """使用与 hyw 渲染器一致的 PIL 压缩策略（可按宽度缩放）。"""
...
async def process_images(mc: MessageChain, vision_model: Optional[str] = None) -> Tuple[List[str], Optional[str]]:
    ...
    if has_images:
        ...
        # 先转 base64，再用 PIL 压缩（质量 + 最大宽度）
        for img in raw_images:
            b64_raw = base64.b64encode(img).decode('utf-8')
            b64_compressed = compress_image_b64(b64_raw, quality=90, max_width=2048)
