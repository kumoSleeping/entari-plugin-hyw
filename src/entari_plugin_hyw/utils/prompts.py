...
- 搜索引用: 在正文中使用 `ref:数字id` (代码形式) 如 `ref:1` 标注来源, 并在文末通过 `references` 代码块列出. 禁止合并引用(如 `ref:1,2`), 必须分开标注.
...
[最终回复格式]
- 工具引用: 在正文中使用 `mcp:字母顺序` (代码形式) 如 `mcp:a` 标注来源, 并在文末通过 `mcp` 代码块列出. 禁止合并引用, 必须分开标注.
